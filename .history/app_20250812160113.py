...
# 简易蒲福风级估算（km/h）
def estimate_beaufort_kmh(speed_kmh: float) -> int:
    try:
        s = float(speed_kmh or 0)
    except Exception:
        s = 0
    # 范围按 km/h 近似
    if s < 1: return 0
    ...
    try:
        ...
        for item in root.iter('item'):
            ...
            if title_el is not None and title_el.text:
                ...
                if t and not _is_excluded_title(t):
                    titles.append(t)
...
async def get_news_content():
    ...
    if cached_news is not None:
        print("使用缓存的新闻内容")
        return cached_news, cached_items, cached_date, None
    ...
    try:
        async with AsyncWebCrawler() as crawler:
            ...
            aggregated_items = []  # 展示文本（含来源）
            aggregated_links = []  # 点击链接
            seen = set()
            ...
            for src, res in zip(SHIPPING_SOURCES, results):
                ...
                if md:
                    ...
                    for raw in lines:
                        ...
                        if _is_excluded_title(title):
                            continue
                        per_titles.append(title)
                        per_links.append(url)
                        if len(per_titles) >= 8:
                            break
                ...
                if len(per_titles) < 2:
                    ...
                    rss_titles = []
                    rss_links = []
                    feed_url = SOURCE_FEEDS.get(host)
                    if feed_url:
                        try:
                            resp = requests.get(feed_url, timeout=10)
                            if resp.status_code == 200:
                                ...
                                for item in root.iter('item'):
                                    ...
                                    link_val = (link_el.text or '').strip() if link_el is not None else ''
                                    if title_val and not _is_excluded_title(title_val):
                                        rss_titles.append(title_val)
                                        rss_links.append(link_val)
                                    if len(rss_titles) >= 8:
                                        break
                        except Exception:
                            pass
                    if rss_titles:
                        per_titles, per_links = rss_titles, rss_links

                # 汇总，做去重与翻译
                for title, url in zip(per_titles, per_links):
                    key = _normalize_headline(title)
                    if not key or key in seen:
                        continue
                    seen.add(key)
                    zh_title = _translate_to_zh(title)
                    display = f"{zh_title}（来源：{src['name']}）"
                    aggregated_items.append(display)
                    aggregated_links.append(url)
            ...
            clear_old_cache()
            return formatted_news, news_items, date_str, aggregated_links
...
def get_news():
    ...
    try:
        ...
        news_content, news_items, date_str, links = loop.run_until_complete(get_news_content())
        loop.close()
...
def get_weather():
    ...
    try:
        ...
        # 海域风况
        marine_area = None
        for k, v in MARINE_AREA_MAP.items():
            if k in area_name or k in loc:
                marine_area = v
                break
        marine = None
        if marine_area:
            bf = estimate_beaufort_kmh(wind_kmph)
            marine = {
                'area': marine_area,
                'windKmph': wind_kmph,
                'windDir': wind_dir,
        ...
        return jsonify({
            'success': True,
            'location': area_name,
            'current': {'tempC': temp_c, 'desc': desc, 'windKmph': wind_kmph, 'windDir': wind_dir},
            'forecast': forecast,
            'marine': marine
        })
    except Exception as e:

...
async function loadWeatherTicker(force = false) {
    try {
        ...
        const container = document.getElementById('weatherTicker');
        if (el && container) {
            // 重置动画以便立即生效
            ...
            // 兜底：若元素宽度为0或动画未生效，直接静态展示
            setTimeout(() => {
                try {
                    const running = getComputedStyle(el).animationName !== 'none';
                    if (el.offsetWidth === 0 || !running) {
                        el.style.animation = 'none';

...
async function loadWeatherTicker() {
    try {
        ...
        if (!tickerEl) return;
        tickerEl.textContent = '正在获取天气...';
        const resp = await fetch('/aizaobao/api/weather');
        const data = await resp.json();
        if (data.success) {
            currentWeatherText = data.text;
            renderWeatherTicker(data.text);
        } else {
            tickerEl.textContent = '天气获取失败';
        }
    } catch (e) {
        const tickerEl = document.getElementById('weatherTicker');
        if (tickerEl) tickerEl.textContent = '天气获取失败';
    }
}
...
async function saveSettings() {
    const config = {
        ...
        vol: parseFloat(document.getElementById('vol') ? .value || '1.0'),
        weather_location: document.getElementById('weatherLocation') ? .value || '上海'
    };
...
function resetSettings() {
    if (confirm('确定要重置所有设置吗？')) {
        ...
        document.getElementById('weatherLocation').value = '上海';

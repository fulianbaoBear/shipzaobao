    ...
    try:
        async with AsyncWebCrawler() as crawler:
            ...
            for src, res in zip(SHIPPING_SOURCES, results):
                ...
                per_source = _extract_headlines_for_source(md, src['url'], max_items=8) if md else []
                # 若抓取为空，则尝试RSS兜底
                if len(per_source) < 2:
                    rss_titles = _rss_fallback_titles(host, max_items=8)
                    if rss_titles:
...
def get_weather():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'location': area_name,
            'current': {'tempC': temp_c, 'desc': desc},
            'forecast': forecast
        })
    except Exception as e:

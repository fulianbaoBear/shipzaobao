...
async def get_news_content():
    ...
    try:
        ...
        try:
            ...
            print(f"Playwright抓取失败，将使用后备方案: {e}")
            results = []
        
        for idx, src in enumerate(SHIPPING_SOURCES):
            md = ''
            ...
            # 若为空则使用后备requests+bs4
            if not per_source:
                per_source = _fallback_extract_source(src['url'], max_items=8)
...
def get_weather():
    """获取天气预报（使用 wttr.in JSON），返回简要滚动条文本"""
    try:
        location = request.args.get('location')
        if not location:
            cfg = get_user_config()
            location = cfg.get('weather_location') or '上海'
        key = f"{location}"
        from datetime import datetime, timedelta
        now = datetime.now()
        # 10 分钟内命中缓存
        if _weather_cache['key'] == key and _weather_cache['ts'] and (now - _weather_cache['ts']).seconds < 600:
            return jsonify({'success': True, 'location': location, 'text': _weather_cache['data']})
        # 请求 wttr.in JSON
        url = f"https://wttr.in/{quote(location)}?format=j1"
        resp = requests.get(url, timeout=10)
        if resp.status_code != 200:
            return jsonify({'success': False, 'message': f'天气接口错误: {resp.status_code}'})
        data = resp.json()
        # 解析今日简要
        current = data.get('current_condition', [{}])[0]
        tempC = current.get('temp_C')
        desc = (current.get('weatherDesc', [{}])[0].get('value') if current.get('weatherDesc') else None)
        feels = current.get('FeelsLikeC')
        wind = current.get('windspeedKmph')
        humidity = current.get('humidity')
        text = f"{location} 天气：{desc or ''} 气温{tempC}℃ 体感{feels}℃ 风速{wind}km/h 湿度{humidity}%"
        _weather_cache['key'] = key
        _weather_cache['data'] = text
        _weather_cache['ts'] = now
        return jsonify({'success': True, 'location': location, 'text': text})
    except Exception as e:
